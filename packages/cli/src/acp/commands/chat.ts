...
export class SaveChatCommand implements Command {
  ...
  async execute(
    ...
    const { config, geminiClient } = context.agentContext;
...
export class ResumeChatCommand implements Command {
  ...
  async execute(
    ...
    const currentAuthType =
      context.agentContext.config.getContentGeneratorConfig()?.authType;
    if (
      checkpoint.authType &&
      currentAuthType &&
    ...
    const { geminiClient } = context.agentContext;
    geminiClient.setHistory(checkpoint.history);
